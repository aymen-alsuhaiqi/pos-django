...
# from weasyprint import HTML, CSS       # For generating PDFs from HTML strings
...
def receipt_pdf_view(request, sale_id):
    ...
    """
    # Fetch the Sale and its details
    sale = Sale.objects.get(id=sale_id)
    details = SaleDetail.objects.filter(sale=sale)

    # Prepare HttpResponse as a PDF
    ...
    # Example: Write the sale info
    p.drawString(50, 800, f"Receipt for Sale ID: {sale.id}")
    p.drawString(50, 780, f"Customer: {sale.customer}")

    # Example: List each item on the sale
    y_position = 760
    for detail in details:
        line_text = f"{detail.product.name} (Qty: {detail.quantity}) - {detail.total_detail}"
        p.drawString(50, y_position, line_text)
        y_position -= 20
